...
def get_scanner_and_lock_status(client, webapp_id):
    """Fetch scanner details and lock status for a web app using GET /get/was/webapp/<id>."""
    ...
    try:
        ...
        # Extract lock status
        scanner_locked = root.find(".//scannerLocked")
        is_locked = scanner_locked.text.lower() == "true" if scanner_locked is not None else "unknown"
        
        logger.info(f"Retrieved scanner: {scanner_name}, locked: {is_locked} for web app ID: {webapp_id}")
        return scanner_name, is_locked
    except ET.ParseError:
...
def process_url(client, webapp_url, current_url_index, total_urls):
    ...
    try:
        ...
        return {
            'url': webapp_url,
            'scanner_name': 'Unknown',
            'scanner_locked': 'Error'
        }

def process_csv(client, batch_size=10, batch_delay=5):
    ...
    try:
        ...
        for i in range(0, total_urls, batch_size):
            ...
            for j, webapp_url in enumerate(batch_urls, start=i + 1):
                result = process_url(client, webapp_url, j, total_urls)
        ...
        output_csv = os.path.join(base_dir, 'scanner_lock_status.csv')
        results_df = pd.DataFrame(results, columns=['url', 'scanner_name', 'scanner_locked'])
...
def print_results(results):
    ...
    for result in results:
        ...
        logger.info(f"Scanner Name: {result['scanner_name']}")
        print(f"Scanner Locked: {result['scanner_locked']}")
        logger.info(f"Scanner Locked: {result['scanner_locked']}")
        print("-" * 50)
        logger.info("-" * 50)
    
    print(f"Total URLs processed: {len(results)}")
    logger.info(f"Total URLs processed: {len(results)}")
    print(f"Results saved to scanner_lock_status.csv")
    logger.info(f"Results saved to scanner_lock_status.csv")

def run_processing(username, password, output_text, status_label):
    ...
    try:
        ...
        results = process_csv(client, batch_size=10, batch_delay=5)
...
def main_gui():
    ...
    root = tk.Tk()
    root.title(f"Qualys Scanner Lock Status v{SCRIPT_VERSION}")
    ...
    def start_processing():
        ...
        threading.Thread(
            ...
            args=(username, password, output_text, status_label),
            daemon=True
        ).start()
...
def main_cli():
    """CLI main function."""
    logger.info(f"Starting Qualys Scanner Lock Status Script v{SCRIPT_VERSION}")
    print(f"[INFO] Starting Qualys Scanner Lock Status Script v{SCRIPT_VERSION}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Qualys Scanner Lock Status Script")
